 ...
 *
 * Enable or disable bus drivers used in the factory class. Disable to reduce
 * code size. Set to `1` to enable, `0` to disable.
 */
 ...
 * When set to `0`, code for unused bus drivers will be excluded to speed up
 * compilation. At this time, users should ensure that the bus driver is not
 * used.
 *
 * Example with SPI:
 * (CONF1 = ESP_PANEL_DRIVERS_BUS_USE_SPI, CONF2 =
 * ESP_PANEL_DRIVERS_BUS_COMPILE_UNUSED_DRIVERS)
 *
 * | CONF1 | CONF2 | Driver Available ( = CONF1 ^ CONF2) | Factory Support ( =
 * CONF1) |
 * |-------|-------|-------------------------------------|----------------------------|
 * |   0   |   0   |                 No                  |              No | |
 * 1   |   0   |                 Yes                 |              Yes | |   0
 ...
 *
 * Enable or disable LCD drivers used in the factory class. Disable to reduce
 * code size. Set to `1` to enable, `0` to disable.
 */
 ...
 * When set to `0`, code for unused LCD drivers will be excluded to speed up
 * compilation. At this time, users should ensure that the LCD driver is not
 * used.
 *
 * Example with ILI9341:
 * (CONF1 = ESP_PANEL_DRIVERS_LCD_USE_ILI9341, CONF2 =
 ...
 */
#define ESP_PANEL_DRIVERS_TOUCH_MAX_POINTS                                     \
  (10) // Maximum number of touch points supported
#define ESP_PANEL_DRIVERS_TOUCH_MAX_BUTTONS                                    \
  (5) // Maximum number of touch buttons supported

/**
 * @brief Touch driver availability
 *
 * Enable or disable touch drivers used in the factory class. Disable to reduce
 * code size. Set to `1` to enable, `0` to disable.
 */
 ...
 * When set to `0`, code for unused touch drivers will be excluded to speed up
 * compilation. At this time, users should ensure that the touch driver is not
 * used.
 *
 * Example with GT911:
 * (CONF1 = ESP_PANEL_DRIVERS_TOUCH_USE_GT911, CONF2 =
...
#define ESP_PANEL_DRIVERS_TOUCH_XPT2046_Z_THRESHOLD                            \
  (400) // Minimum pressure threshold for touch detection

/**
 * @brief Enable interrupt (PENIRQ) output mode
 *
 * When enabled, XPT2046 outputs low on PENIRQ when touch is detected (Full
 * Power Mode). Consumes more power but provides interrupt capability.
 */
 ...
 *
 * When enabled, internal Vref remains on between conversions. Slightly higher
 * power consumption, but requires fewer transactions for battery voltage, aux
 ...
 * When disabled, `process_coordinates` must be called manually to convert
 * values.
 */
...
#endif // ESP_PANEL_DRIVERS_TOUCH_USE_XPT2046 ||
       // ESP_PANEL_DRIVERS_TOUCH_COMPILE_UNUSED_DRIVERS
 ...
 *
 * Enable or disable IO Expander drivers used in the factory class. Disable to
 * reduce code size. Set to `1` to enable, `0` to disable.
 */
 ...
 *
 * Enable or disable backlight drivers used in the factory class. Disable to
 * reduce code size. Set to `1` to enable, `0` to disable.
 */
 ...
 * When set to `0`, code for unused backlight drivers will be excluded to speed
 * up compilation. At this time, users should ensure that the backlight driver
 * is not used.
 *
 * Example with PWM_LEDC:
 * (CONF1 = ESP_PANEL_DRIVERS_BACKLIGHT_USE_PWM_LEDC, CONF2 =
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 * Do not change the following versions. These version numbers are used to check
 * compatibility between this configuration file and the library. Rules for
 * version numbers:
 * 1. Major version mismatch: Configurations are incompatible, must use library
 * version
 * 2. Minor version mismatch: May be missing new configurations, recommended to
 * update
