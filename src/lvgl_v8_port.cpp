...
copy_pixel_8bpp(uint8_t *to, const uint8_t *from) {
  *to++ = *from++;
}

__attribute__((always_inline)) static inline void
copy_pixel_16bpp(uint8_t *to, const uint8_t *from) {
  *(uint16_t *)to++ = *(const uint16_t *)from++;
}

__attribute__((always_inline)) static inline void
copy_pixel_24bpp(uint8_t *to, const uint8_t *from) {
  *to++ = *from++;
  *to++ = *from++;
  *to++ = *from++;
}
...
#define ROTATE_90_ALL_BPP()                                                    \
  {                                                                            \
    ...
    for (int from_y = y_start; from_y < y_end + 1; from_y++) {                 \
      from_index =                                                             \
          from_y * from_bytes_per_line + x_start * from_bytes_per_piexl;       \
    ...
    for (int i = 0; i < h; i += block_h) {                                     \
      ...
      for (int j = 0; j < w; j += block_w) {                                   \
        ...
        for (int x = i; x < max_height; x++) {                                 \
          ...
          for (int y = j, mirrored_y = start_y; y < max_width;                 \
               y += 4, mirrored_y -= 4) {                                      \
...
#define ROTATE_180_ALL_BPP()                                                   \
  {                                                                            \
    ...
    to_index_const =                                                           \
        (h - 1) * to_bytes_per_line + (w - x_start - 1) * to_bytes_per_piexl;  \
    for (int from_y = y_start; from_y < y_end + 1; from_y++) {                 \
      from_index =                                                             \
          from_y * from_bytes_per_line + x_start * from_bytes_per_piexl;       \
...
#define ROTATE_270_ALL_BPP()                                                   \
  {                                                                            \
    ...
    to_index_const =                                                           \
        x_start * to_bytes_per_line + (h - 1) * to_bytes_per_piexl;            \
    for (int from_y = y_start; from_y < y_end + 1; from_y++) {                 \
...
rotate_copy_pixel(const uint8_t *from, uint8_t *to, uint16_t x_start,
                  uint16_t y_start, uint16_t x_end, uint16_t y_end, uint16_t w,
...
static void flush_dirty_copy(void *dst, void *src,
  ...
  for (int i = 0; i < dirty_area->inv_p; i++) {
    ...
    if (dirty_area->inv_area_joined[i] == 0) {
      ...
      rotate_copy_pixel((uint8_t *)src, (uint8_t *)dst, x_start, y_start, x_end,
                        y_end, LV_HOR_RES, LV_VER_RES,
...
static void flush_callback(lv_disp_drv_t *drv, const lv_area_t *area,
  ...
  if (lv_disp_flush_is_last(drv)) {
    ...
    if (drv->full_refresh) {
      ...
      rotate_copy_pixel((uint8_t *)color_map, (uint8_t *)next_fb, offsetx1,
                        offsety1, offsetx2, offsety2, LV_HOR_RES, LV_VER_RES,
                        LVGL_PORT_ROTATION_DEGREE);
...
void flush_callback(lv_disp_drv_t *drv, const lv_area_t *area,
  ...
  lcd->drawBitmap(offsetx1, offsety1, offsetx2 - offsetx1 + 1,
                  offsety2 - offsety1 + 1, (const uint8_t *)color_map);
...
static void update_callback(lv_disp_drv_t *drv) {
  ...
  ESP_UTILS_LOGD("Current mirror x: %d, mirror y: %d, swap xy: %d",
                 disp_init_mirror_x, disp_init_mirror_y, disp_init_swap_xy);
#endif
}
...
static lv_disp_t *display_init(LCD *lcd) {
  ...
  for (int i = 0; (i < LVGL_PORT_BUFFER_NUM) && (i < LVGL_PORT_BUFFER_NUM_MAX);
       i++) {
  ...
  // for LVGL rendering and another for LCD refresh
  buffer_size = lcd_width * lcd_height;
#if (LVGL_PORT_DISP_BUFFER_NUM >= 3) && (LVGL_PORT_ROTATION_DEGREE == 0) &&    \
    LVGL_PORT_FULL_REFRESH

  // With the usage of three buffers and full-refresh, we always have one buffer
  // available for rendering, eliminating the need to wait for the LCD's sync
  // signal
  ...
  for (int i = 0;
       (i < LVGL_PORT_DISP_BUFFER_NUM) && (i < LVGL_PORT_BUFFER_NUM_MAX); i++) {
...
bool lvgl_port_init(LCD *lcd, Touch *tp) {
  ...
  ESP_UTILS_CHECK_FALSE_RETURN(
      (bus_type == ESP_PANEL_BUS_TYPE_RGB) ||
          (bus_type == ESP_PANEL_BUS_TYPE_MIPI_DSI),
      false, "Avoid tearing function only works with RGB/MIPI-DSI LCD now");
  ESP_UTILS_LOGI("Avoid tearing is enabled, mode: %d, rotation: %d",
                 LVGL_PORT_AVOID_TEARING_MODE, LVGL_PORT_ROTATION_DEGREE);
#endif
  ...
  lv_init();
#if !LV_TICK_CUSTOM
  ESP_UTILS_CHECK_FALSE_RETURN(tick_init(), false,
                               "Initialize LVGL tick failed");
#endif

  ESP_UTILS_LOGI("Initializing LVGL display driver");
  disp = display_init(lcd);
  ESP_UTILS_CHECK_NULL_RETURN(disp, false,
                              "Initialize LVGL display driver failed");
  ...
  if (bus_type != ESP_PANEL_BUS_TYPE_RGB) {
    ...
    lcd->attachDrawBitmapFinishCallback(onDrawBitmapFinishCallback,
                                        (void *)disp->driver);
  }
  ...
  if (tp != nullptr) {
    ...
    indev = indev_init(tp);
    ESP_UTILS_CHECK_NULL_RETURN(indev, false,
                                "Initialize LVGL input driver failed");
...
#if LVGL_PORT_AVOID_TEAR
  lcd->attachRefreshFinishCallback(onLcdVsyncCallback,
...
#if !LV_TICK_CUSTOM
  ESP_UTILS_CHECK_FALSE_RETURN(tick_deinit(), false,
                               "Deinitialize LVGL tick failed");
#endif
